...
def transform(G, motif_list, _path):
    ...
    transfer_triads = sm.collapse(G, motif_list[2])
    end_t = time.time()
    time_delta = round(end_t - start_t, 3)
...
def main():
    ...
    # # To get target graph_name through todolist.txt
    # with open('todolist.txt') as f:
    #     _name_list = [x.strip() for x in f.readlines()]
    #
    # # To get the graph files' directory
    # for _name in _name_list:
    #     _filepath = []
    #     for dirpath, dirnames, filenames in os.walk(root + _name):  # walk through a certain graph directory
    #         # To get the original graph file, start with 'out.' and a name followed(not always the graph_name)
    ...
    for _name, _filename in zip(name_list, ori_graph_path):
        ...
        try:
            print("Start loading...")
            G0 = gu.load_edgelist(root + _name + _filename, _name=_name,)
            G_after = gu.load_edgelist(root + _name + _filename, _name=_name, )
            ...
            out_report = [_name, stats_G0[0], stats_G0[1], time_G0, time_Gaf, time_delta, time_redu,
                          ...
                          num_fans, avg_fan_size, num_conn2, num_traids, det_res[-1], trans_res[-1]]
            with open('konect_SME_report-new.csv', 'a', newline="") as f:

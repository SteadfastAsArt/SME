...
def training(G, _filepath, o=1, number_walks=10, walk_length=80, representation_size=64, window_size=5):
    ...
    if o == 1:
        model.wv.save_word2vec_format(_filepath + G.name + '.emb')
    else:
        model.wv.save_word2vec_format(_filepath + G.name + '0.emb')
    time_end = time.time()
